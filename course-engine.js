...
function advance() {
  ...
  if (STATE.moduleIdx >= COURSE.length - 1) {
    ...
    document.getElementById('completeProgress').innerHTML = '🎉 <strong>All 7 modules complete!</strong> Your certificate is ready.';
...
function bindSidebarClicks() {
  document.querySelectorAll('.sidebar .module').forEach((m, idx) => {
    m.addEventListener('click', () => {
      // Can switch to any module that is current or already completed
      const isCompleted = STATE.completedModules.has(idx);
      const isCurrent = idx === STATE.moduleIdx;
      const isNext = idx === Math.max(...STATE.completedModules) + 1;
      if (!isCompleted && !isCurrent && !isNext) {
        window.FWA.toast({ icon: '🔒', title: 'Module Locked', msg: 'Complete earlier modules first', variant: 'wrong' });
        return;
      }
      STATE.moduleIdx = idx;
      STATE.videoIdx = 0;
